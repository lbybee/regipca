...
cdef double lasso_iter(double[::1] w, double[::1,:] XFXF, double[::1] XFy,
                       double[::1] stp_size, double[::1] grad,
                       double[::1] grad_update, double[::1] regconst,
    ...
    for l in range(L):
        ...
        for k in range(K):
            ...
            # TODO this procedure is likely very inefficient
            # add the NEGATIVE gradient - (XFXFw - XFy)
            prox = (w[l*K+k] * stp_size[l] - grad[l*K+k])

            # generate slice fitted values
            _dgemv("n", LK, 1, 1., &XFXF[0, l*K+k], LK, &w[l*K+k], 1,
                   0, &grad_update[0], 1)
            ...
            # build group penalty
            gprox = (1. - (regconst[l] / fabs(prox)))
            gprox = max(0, gprox)

            w_lk = w[l*K+k]
            w[l*K+k] = prox * gprox / stp_size[l]
...
def lasso(double[::1] w, double[::1,:] XFXF, double[::1] XFy,
    ...
    with nogil:
        for i in range(gliter):
            ...
            # update est
            score = lasso_iter(w, XFXF, XFy, stp_size, grad, grad_update,
                               regconst, L, K, LK)
